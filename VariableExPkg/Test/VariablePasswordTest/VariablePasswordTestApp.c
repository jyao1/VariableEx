...
{
  EDKII_VARIABLE_EX_PROTOCOL            *Variable;
  EFI_STATUS                            Status;
  ...
  Status = gBS->LocateProtocol(&gEdkiiVariableExProtocolGuid, NULL, (VOID **)&Variable);
  ASSERT_EFI_ERROR(Status);

  DEBUG((EFI_D_INFO, "Test 1: Create PASSWORD_AUTH variable\n"));
  Status = Variable->SetVariableEx (
